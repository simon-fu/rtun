...
- 本期不实现“基于候选特征/历史失败模式”的动态 `role=auto`
- 保留当前静态 `role=auto`（initiator=`nat3` / responder=`nat4`）
- 优先收敛参数安全上限、运维文档、排障文档与测试矩阵

### 待办事项

- [x] 评估是否需要 `role=auto`（基于候选特征/历史失败模式；本期结论：暂不启用动态 auto）
- [x] 若实现 `auto`，明确误判回退策略（本期记录未来启用时的降级策略，当前未启用）
- [x] 增加运维文档：参数说明、推荐配置、调试方法（`rtun/docs/hardnat-ops.md`）
- [x] 增加排障文档：常见失败原因与日志定位路径（`rtun/docs/hardnat-troubleshooting.md`）
- [x] 增加端到端测试矩阵（off/fallback/assist；文档矩阵先落地）
- [x] 收敛默认值和资源上限（防止误用）

### 验收项

- [x] 文档完整，运维可独立启用/排障（人工检查通过）
- [x] 自动策略开启时不会破坏默认路径稳定性（本期未启用动态 auto，默认路径不变）
- [ ] 关键场景测试可复现
...
- [x] Milestone 6 实施完成（人工检查通过；关键场景实网 smoke 待补）
