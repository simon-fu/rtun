...
impl<O> KcpWriteBuf<O> {
    ...
    pub fn pop_front(&mut self) -> Option<bytes::Bytes> {
        self.que.pop_front()
    }

    pub fn push_front(&mut self, data: bytes::Bytes) {
        self.que.push_front(data);
    }
...
impl<O: KcpBufOps> KcpWrite for KcpWriteBuf<O> {
    ...
    fn begin_segment<'a>(&'a mut self) -> Self::Buf<'a> {
        if self.buf.len() == 0 {
            self.ops.on_begin_packet(&mut self.buf);
...
// pub struct KcpBufRef<'a>(&'a mut BytesMut);
...
//     pub fn data<'b: 'a>(&'b self) -> &'a [u8] {
//         &self.0[..]
//     }

//     pub fn to_owned(&mut self) -> KcpBuf {
//         KcpBuf(self.0.split())
//     }
// }

// impl<'a> Drop for KcpBufRef<'a> {
//     fn drop(&mut self) {
//         self.0.clear();
//     }
// }
...
// pub trait KcpWrite {
//     fn write_kcp_packet<'a>(&mut self, packet: KcpBufRef<'a>) -> io::Result<()>;
// }

// #[derive(Default)]
// struct KcpOutput<O: KcpWrite>(O);

// impl<O: KcpWrite> KcpWrite for KcpOutput<O> {
//     #[inline]
//     fn write_kcp_packet<'a>(&mut self, packet: KcpBufRef<'a>) -> io::Result<()> {
//         trace!("[RO] {} bytes", packet.len());
//         self.0.write_kcp_packet(packet)
//     }
// }
...
impl<Output: KcpWrite> Debug for Kcp<Output> {
...
impl<Output: KcpWrite> Kcp<Output> {
    ...
    pub fn output_mut<'a>(&'a mut self) -> &'a mut Output {
        // &mut self.output.0
        &mut self.buf
    }
    ...
    fn _flush_ack(&mut self, segment: &mut KcpSegment) -> KcpResult<()> {
        ...
        for &(sn, ts) in &self.acklist {
            if self.buf.len() + KCP_OVERHEAD as usize > self.mtu as usize {
                self.buf.flush_packet();
                // self.output.write_kcp_packet(KcpBufRef(&mut self.buf))?;
                // // self.output.write_all(&self.buf)?;
                // // self.buf.clear();
            }
            segment.sn = sn;
            segment.ts = ts;
            segment.encode(&mut self.buf.begin_segment());
        }
        self.acklist.clear();
    ...
    pub fn flush(&mut self) -> KcpResult<()> {
        ...
        for snd_segment in &mut self.snd_buf {
            ...
            if need_send {
                ...
                snd_segment.encode(&mut self.buf.begin_segment());
