...
use super::{
    agent::ctrl::{make_agent_ctrl, AgentCtrl},
    ctrl_service::{spawn_ctrl_service, ExitReason},
    switch_pair::SwitchPair,
    switch_sink::PacketSink,
    switch_source::PacketSource,
};

pub async fn make_agent_session<S1: PacketSink, S2: PacketSource>(
    switch_session: SwitchPair<S1, S2>,
) -> Result<AgentSession<S1, S2>> {
    let uid = gen_huid();
