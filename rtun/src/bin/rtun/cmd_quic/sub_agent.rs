...
pub async fn run(args: CmdArgs) -> Result<()> {
    let endpoint = quinn_util::try_listen(
        &args.listen,
    ...
    let shared: AShared = Arc::new(Shared { args });
...
async fn handle_connection(shared: AShared, conn: quinn::Connecting, uid: HUId) -> Result<()> {
    ...
    {
        let req: AuthRequest =
            xfer_util::read_short_json(&mut session.rx, &mut session.rbuf).await?;
...
pub struct CmdArgs {
    ...
    #[clap(long = "secret", long_help = "access secret")]
