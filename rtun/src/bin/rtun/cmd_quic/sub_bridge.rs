...
pub async fn run(args: CmdArgs) -> Result<()> {
    ...
    let mut server_crypto = try_load_quic_cert(
...
impl Session {
    async fn handle_req_register(&mut self, shared: &AShared, req: RequestRegister) -> Result<()> {
        ...
        let room = {
            ...
            data.rooms.entry(req.room)
            .or_insert_with_key(|_key|Arc::new(Room {
                // name: key.clone(),
                data: Default::default(),
            }))
            .clone()
    ...
    async fn handle_req_forward(&mut self, shared: &AShared, req: RequestForward) -> Result<()> {
        let room = {
            shared.data.lock().rooms.get(&req.room).map(|x|x.clone())
        };
        ...
        let agent = {
            room.data.lock().agents.get(&req.target).map(|x|x.clone())
        };
    ...
    async fn loop_next_req(&mut self) -> Result<()> {
        ...
        loop {
            ...
            let req: NextRequest = tokio::time::timeout(timeout, fut).await
...
async fn read_short_packet<R: AsyncReadExt + Unpin, B: BufMut>(rx: &mut R, buf: &mut B) -> Result<usize> {
