...
use crate::rest_proto::{make_pub_sessions, make_sub_url, make_ws_scheme, AgentInfo};

pub async fn client_select_url(
    url_str: &str,
    agent: Option<&str>,
    secret: Option<&str>,
) -> Result<url::Url> {
    let mut url = url::Url::parse(url_str).with_context(|| "invalid url")?;
