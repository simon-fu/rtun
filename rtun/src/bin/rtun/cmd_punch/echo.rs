...
pub async fn kick_echo_server(listen_addr: SocketAddr) -> Result<()> {
    let socket = UdpSocket::bind(listen_addr)
        .await
        .with_context(|| "bind echo server socket failed")?;
...
pub async fn kick_echo_client(target: SocketAddr) -> Result<()> {
    ...
    socket
        .connect(target)
        .await
        .with_context(|| "echo client connect failed")?;
    let socket = Arc::new(socket);

    {
        let span = span!(parent: None, Level::DEBUG, "echo-recv");
        let socket = socket.clone();
        tokio::spawn(async move { echo_recv(socket).await }.instrument(span));
    }
