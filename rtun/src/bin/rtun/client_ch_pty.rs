...
use rtun::{
    ...
    proto::{
        pty_input_packet::Pty_input_args, PtyInputPacket, PtyOutputPacket, PtyResizeArgs,
        ShutdownArgs,
    },
...
impl PtyChReceiver {
    ...
    pub async fn recv_packet(&mut self) -> Result<PtyOutputPacket> {
        ...
        Ok(PtyOutputPacket::parse_from_tokio_bytes(&packet.payload)
...
impl PtyChSender {
    ...
    pub async fn send_event(&self, ev: PtyEvent) -> Result<()> {
        let data = match ev {
            PtyEvent::StdinData(data) => se_pty_stdin_packet(data)?,
            PtyEvent::Resize(size) => se_pty_resize_packet(size)?,
        };

        self.ch_tx
            .send_data(data)
            .await
            .map_err(|_x| anyhow!("send data failed"))
    }
}
...
pub async fn process_recv_result(result: Result<PtyOutputPacket>) -> Result<Option<ShutdownArgs>> {
    ...
    let args = result
        .with_context(|| "recv packet failed")?
        // .with_context(||"channel closed")?
        .pty_output_args
        .with_context(|| "empty pty_output_args")?;
