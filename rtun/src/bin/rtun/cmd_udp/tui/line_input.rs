...
use super::term_theme_renderer::TermThemeRenderer;
...
impl<'a, T> Input<'a, T> {
    ...
    pub fn with_theme(_theme: &'a dyn Theme) -> Self {
        Self {
            prompt: "".into(),
    ...
    <T as FromStr>::Err: Debug + ToString,
{
    ...
    // pub fn interact_text(&mut self, state: &mut InputState, key: Key) -> io::Result<T> {
    ...
    pub fn init_state(&self, state: &mut InputState, term: &Term, render: &mut TermThemeRenderer<'_>) -> io::Result<()> {
        ...
        *state = InputState {
            ...
            // #[cfg(feature = "history")]
            hist_pos: 0,
            prompt_len: render.input_prompt(
                &self.prompt,
        ...
        if let Some(initial) = self.initial_text.as_ref() {
            ...
            state.chars = initial.chars().collect();
            state.position = state.chars.len();
        }
        term.flush()?;
    ...
    pub fn restore_state(&mut self, state: &mut InputState, term: &Term, render: &mut TermThemeRenderer<'_>) -> io::Result<()> {
        ...
        if state.chars.len() > 0 {
            let input = state.chars.iter().collect::<String>();
            term.write_str(&input)?;
            term.move_cursor_left(input.len() - state.position)?;
        }
        term.flush()?;
        Ok(())
    }
    ...
    pub fn interact_text_on(&mut self, state: &mut InputState, term: &Term, key: Key) -> io::Result<bool> {
        ...
        // #[cfg(feature = "history")]
        let hist_pos = &mut state.hist_pos;
