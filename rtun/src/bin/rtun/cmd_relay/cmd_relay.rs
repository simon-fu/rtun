...
const DEFAULT_P2P_HARDNAT_SOCKET_COUNT: u32 = 64;
const DEFAULT_P2P_HARDNAT_SCAN_COUNT: u32 = 64;
const DEFAULT_P2P_HARDNAT_INTERVAL_MS: u64 = 1000;
const DEFAULT_P2P_HARDNAT_BATCH_INTERVAL_MS: u64 = 5000;
const DEFAULT_P2P_HARDNAT_ASSIST_DELAY_MS: u64 = 300;
const AGENT_SCRIPT_STDOUT_LIMIT: u32 = 256 * 1024;
const AGENT_SCRIPT_STDERR_LIMIT: u32 = 256 * 1024;

#[derive(Debug, Clone, Copy, clap::ValueEnum, PartialEq, Eq)]
enum RelayHardNatModeCli {
    Off,
    Fallback,
    Assist,
    Force,
...
#[derive(Debug, Clone, Copy, clap::ValueEnum, PartialEq, Eq)]
enum RelayHardNatRoleCli {
    Auto,
    Nat3,
    Nat4,
}

impl RelayHardNatRoleCli {
    fn as_str(self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Nat3 => "nat3",
...
struct RelayUdpHardNatConfig {
    mode: RelayHardNatModeCli,
    role: RelayHardNatRoleCli,
    socket_count: u32,
    scan_count: u32,
    interval_ms: u32,
...
impl RelayUdpHardNatConfig {
    ...
    fn is_assist(self) -> bool {
        self.mode == RelayHardNatModeCli::Assist
    }
...
fn resolve_relay_udp_hard_nat_config(args: &CmdArgs) -> Result<RelayUdpHardNatConfig> {
    ...
    let assist_delay_ms = u32::try_from(args.p2p_hardnat_assist_delay).with_context(|| {
        format!(
            "p2p hardnat assist delay too large [{}ms]",
            args.p2p_hardnat_assist_delay
        )
    })?;
...
async fn do_run(args: CmdArgs, argv: Vec<String>) -> Result<()> {
    ...
    let udp_relay_hard_nat = resolve_relay_udp_hard_nat_config(&args)?;
    ...
    tracing::info!(
        "[relay_hardnat_diag] udp relay hard-nat config: mode={}, role={}, socket_count={}, scan_count={}, interval={}ms, batch_interval={}ms, ttl={}, no_ttl={}",
...
async fn run_relay_session<H: CtrlHandler>(
    ...
    for _ in 0..desired {
        ...
        let tunnel = open_udp_relay_tunnel(
            &ctrl,
            target,
            idle_timeout_secs,
            max_payload,
...
async fn open_udp_relay_tunnel<H: CtrlHandler>(
    ...
    let local_nat4_candidate_ips = if udp_relay_hard_nat.is_off() {
        ...
        && local_role_plan.local_role == HardNatRole::Nat3
    {
        match prepare_nat3_public_target("0.0.0.0:0", &[]).await {
            Ok(Some(prepared)) => {
                tracing::warn!(
                    "[relay_hardnat_diag] prepared nat3 public target [{}] for local_role=nat3",
            ...
            Ok(None) => {
                tracing::warn!(
                    "[relay_hardnat_diag] nat3 public target discovery did not confirm a reusable mapped address; fallback to ICE-derived target"
                );
                None
            }
            Err(e) => {
                tracing::warn!(
                    "[relay_hardnat_diag] nat3 public target discovery failed; fallback to ICE-derived target, err [{e:#}]"
                );
                None
            }
        }
    ...
    if udp_relay_hard_nat.is_force() {
        tracing::warn!(
            "[relay_hardnat_diag] udp relay hard-nat force enabled: mode={}, role={}, socket_count={}, scan_count={}, interval={}ms, batch_interval={}ms, assist_delay={}ms, ttl={}, no_ttl={}, target=[{}]; force path will be used instead of ICE dial",
            udp_relay_hard_nat.mode.as_str(),
            udp_relay_hard_nat.role.as_str(),
            udp_relay_hard_nat.socket_count,
            udp_relay_hard_nat.scan_count,
            udp_relay_hard_nat.interval_ms,
            udp_relay_hard_nat.batch_interval_ms,
            udp_relay_hard_nat.assist_delay_ms,
            udp_relay_hard_nat.ttl_display(),
            udp_relay_hard_nat.no_ttl,
            target,
        );
        ...
        tracing::warn!(
            "[relay_hardnat_diag] udp relay hard-nat assist enabled: mode={}, role={}, socket_count={}, scan_count={}, interval={}ms, batch_interval={}ms, assist_delay={}ms, ttl={}, no_ttl={}, target=[{}]; racing ICE dial vs hard-nat dial",
            udp_relay_hard_nat.mode.as_str(),
            udp_relay_hard_nat.role.as_str(),
            udp_relay_hard_nat.socket_count,
            udp_relay_hard_nat.scan_count,
        ...
        tracing::warn!(
            "[relay_hardnat_diag] udp relay hard-nat requested (placeholder only): mode={}, role={}, socket_count={}, scan_count={}, interval={}ms, batch_interval={}ms, assist_delay={}ms, ttl={}, no_ttl={}, target=[{}]; ICE-only path remains active",
            udp_relay_hard_nat.mode.as_str(),
            udp_relay_hard_nat.role.as_str(),
            udp_relay_hard_nat.socket_count,
            udp_relay_hard_nat.scan_count,
    ...
    let local_nat3_public_addrs = local_prepared_nat3
        ...
        .unwrap_or_default();
    let local_hard_nat = build_udp_relay_hard_nat_args(
        udp_relay_hard_nat,
...
fn build_udp_relay_hard_nat_args(
    ...
    let mut args = P2PHardNatArgs {
        ...
        assist_delay_ms: cfg.assist_delay_ms,
...
async fn open_udp_relay_tunnel_assist_socket<H: CtrlHandler>(
    ...
    let hard_nat_delay = Duration::from_millis(cfg.assist_delay_ms as u64);

    let ice_fut = async move {
        let conn = peer
            .dial(remote_ice_for_ice)
            .await
            .with_context(|| format!("udp relay ICE dial failed, target [{}]", target))?;
        let (socket, _cfg, remote_addr) = conn.into_parts();
        socket.connect(remote_addr).await.with_context(|| {
            format!(
                "udp relay ICE connect remote failed, remote [{}], target [{}]",
                remote_addr, target
            )
        })?;
        Ok::<_, anyhow::Error>(RelayAssistOpenSocket::Ice {
    ...
    let hard_nat_fut = async move {
        ...
        Ok::<_, anyhow::Error>(RelayAssistOpenSocket::HardNat {
            socket,
            role,
            local_addr,
            remote_addr,
    ...
    let (winner, winner_socket) = race_assist(hard_nat_delay, ice_fut, hard_nat_fut)
        .await
        .map_err(|errs| {
            anyhow::anyhow!(
    ...
    match winner_socket {
        RelayAssistOpenSocket::Ice {
            socket,
            remote_addr,
        } => {
            tracing::warn!(
                "[relay_hardnat_diag] udp relay assist winner={}: remote=[{}], elapsed={}ms, hardnat_delay={}ms, target=[{}]; loser future canceled",
                winner.as_str(),
                remote_addr,
        ...
        RelayAssistOpenSocket::HardNat {
            ...
            tracing::warn!(
                "[relay_hardnat_diag] udp relay assist winner={}: role={}, local=[{}], remote=[{}], hardnat_elapsed={}ms, race_elapsed={}ms, hardnat_delay={}ms, target=[{}]; loser future canceled",
                winner.as_str(),
                role.as_str(),
                local_addr,
                remote_addr,
...
fn load_agent_scripts(
    args: &CmdArgs,
    runtime_argv: &[String],
) -> Result<Vec<RelayAgentScriptSpec>> {
...
pub struct CmdArgs {
    ...
    #[clap(
        long = "p2p-hardnat",
        long_help = "udp relay hard-nat mode (force/assist implemented; fallback placeholder)",
        value_enum,
        default_value_t = RelayHardNatModeCli::Off
    )]
    p2p_hardnat: RelayHardNatModeCli,

    #[clap(
        long = "p2p-hardnat-role",
        long_help = "udp relay hard-nat role hint",
        value_enum,
        default_value_t = RelayHardNatRoleCli::Auto
    )]
    p2p_hardnat_role: RelayHardNatRoleCli,

    #[clap(
        long = "p2p-hardnat-socket-count",
        long_help = "udp relay hard-nat socket count hint",
    ...
    #[clap(
        ...
        long_help = "udp relay hard-nat scan count hint",
    ...
    #[clap(
        ...
        long_help = "udp relay hard-nat interval in milliseconds",
    ...
    #[clap(
        ...
        long_help = "udp relay hard-nat batch interval in milliseconds",
        default_value_t = DEFAULT_P2P_HARDNAT_BATCH_INTERVAL_MS
    )]
    p2p_hardnat_batch_interval: u64,

    #[clap(
        long = "p2p-hardnat-assist-delay",
        long_help = "udp relay hard-nat assist mode delay before starting hard-nat branch (milliseconds)",
        default_value_t = DEFAULT_P2P_HARDNAT_ASSIST_DELAY_MS
    )]
    p2p_hardnat_assist_delay: u64,

    #[clap(long = "p2p-hardnat-ttl", long_help = "udp relay hard-nat ttl hint")]
    p2p_hardnat_ttl: Option<u32>,

    #[clap(
        long = "p2p-hardnat-no-ttl",
        long_help = "udp relay hard-nat hint to disable ttl pre-send"
    )]
...
mod tests {
    ...
    fn parse_cmd_args_for_test(extra: &[&str]) -> CmdArgs {
        let mut argv = vec![
            "rtun",
            "-L",
            "udp://127.0.0.1:14433?to=127.0.0.1:4433",
    ...
    #[test]
    fn relay_hard_nat_cli_defaults_to_off() {
        let args = parse_cmd_args_for_test(&[]);
        let cfg = resolve_relay_udp_hard_nat_config(&args).unwrap();
        assert_eq!(cfg.mode, RelayHardNatModeCli::Off);
        assert_eq!(cfg.role, RelayHardNatRoleCli::Auto);
        ...
        assert_eq!(
            cfg.assist_delay_ms,
            DEFAULT_P2P_HARDNAT_ASSIST_DELAY_MS as u32
        );
        assert_eq!(cfg.ttl, None);
        assert!(!cfg.no_ttl);
        assert!(
            build_udp_relay_hard_nat_args(cfg, &["8.8.8.8".to_string()], &[])
                .as_ref()
                .is_none()
        );
    }
    ...
    #[test]
    fn relay_hard_nat_cli_parses_fallback_role_ttl_and_no_ttl() {
        let args = parse_cmd_args_for_test(&[
            "--p2p-hardnat",
            "fallback",
            "--p2p-hardnat-role",
        ...
        let cfg = resolve_relay_udp_hard_nat_config(&args).unwrap();
        assert_eq!(cfg.mode, RelayHardNatModeCli::Fallback);
        assert_eq!(cfg.role, RelayHardNatRoleCli::Nat4);
        assert_eq!(cfg.socket_count, 128);
        assert_eq!(cfg.scan_count, 256);
        assert_eq!(cfg.interval_ms, 333);
        assert_eq!(cfg.batch_interval_ms, 4444);
        assert_eq!(cfg.assist_delay_ms, 222);
        assert_eq!(cfg.ttl, Some(7));
        assert!(cfg.no_ttl);

        let proto = build_udp_relay_hard_nat_args(cfg, &["8.8.8.8".to_string()], &[]);
        let proto = proto.as_ref().unwrap();
        assert_eq!(proto.mode, 1);
        assert_eq!(proto.role, 2);
        assert_eq!(proto.socket_count, 128);
        assert_eq!(proto.scan_count, 256);
        assert_eq!(proto.interval_ms, 333);
        assert_eq!(proto.batch_interval_ms, 4444);
    ...
    #[test]
    fn parse_udp_relay_hard_nat_target_addr_accepts_empty_and_valid_socket_addr() {
        let args = UdpRelayArgs::default();
        assert_eq!(parse_udp_relay_hard_nat_target_addr(&args).unwrap(), None);

        let args = UdpRelayArgs {
            hard_nat_target_addr: "203.0.113.20:54321".into(),
            ..Default::default()
        };
        assert_eq!(
            parse_udp_relay_hard_nat_target_addr(&args).unwrap(),
            Some("203.0.113.20:54321".parse().unwrap())
        );
    }
