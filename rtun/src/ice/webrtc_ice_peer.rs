...
impl WebrtcIcePeer {
    ...
    pub async fn gather_until_done(&mut self) -> Result<DtlsIceArgs> {
        self.certificate = if !self.config.disable_dtls {
            Some(Certificate::generate_self_signed(vec![
                "localhost".to_owned()
            ])?)
        } else {
            None
        ...
        Ok(DtlsIceArgs {
            ...
            cert_fingerprint: match &self.certificate {
                Some(certificate) => {
                    Some(make_fingerprint(SHA256_ALG, &certificate.certificate[0].0)?)
                }
                None => None,
            },
        })
    ...
    pub async fn dial(&mut self, remote: DtlsIceArgs) -> Result<IceConn> {
        ...
        let conn = agent
            .dial(cancel_rx, remote.ice.ufrag, remote.ice.pwd)
            .await?;
        tracing::debug!("ice dial connected");

        self.make_conn(conn, true, remote.cert_fingerprint.as_deref())
            .await
    }
    ...
    pub async fn accept(&mut self, remote: DtlsIceArgs) -> Result<IceConn> {
        ...
        let (_cancel_tx, cancel_rx) = mpsc::channel(1);
        let conn = agent
            .accept(cancel_rx, remote.ice.ufrag, remote.ice.pwd)
            .await?;

        tracing::debug!("ice accept connected");

        self.make_conn(conn, false, remote.cert_fingerprint.as_deref())
            .await
    }
        ...
        let conn = match self.certificate.take() {
            Some(certificate) => {
                let remote_fingerprint =
                    remote_fingerprint.with_context(|| "remote fingerprint empty")?;
    ...
    fn add_remote_candidates<'a>(&'a self, remote: &IceArgs) -> Result<&'a Arc<IceAgent>> {
        ...
        for candidate in remote.candidates.iter() {
            ...
            agent
                ...
                .with_context(|| "add remote candidate failed")?;
        }
    ...
    pub async fn kick_and_ugrade_to_kcp(
        self,
        remote: DtlsIceArgs,
        is_client: bool,
    ) -> Result<IceKcpConnection> {
        let mut peer = self;
    ...
    pub async fn kick_and_ugrade_to_mpsc(
        self,
        remote: DtlsIceArgs,
        is_client: bool,
    ) -> Result<IceMpsc> {
