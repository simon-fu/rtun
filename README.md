...
- relay 默认忽略脚本执行失败；可配置为“脚本失败则该 agent 不可用并切换到下一个 agent”。
...
```bash
rtun relay \
  -L udp://0.0.0.0:20999?to=127.0.0.1:10999 \
  "quic://127.0.0.1:8888" \
  --secret sec123 \
  --quic-insecure \
...
- 在尽量不改动现有 relay 数据面的前提下，补齐可观测性能力，降低排障成本。
...
- [ ] S5 回归与文档
  ...
  - README 补充使用示例与排障建议
  - 验收：现有 smoke/e2e 不回归，新增测试通过

完成判定（DoD）：

- `--tui` 在 relay 实战场景可用，且不影响非 TUI 运行路径
- `--log-file` 行为符合“显式开启才落盘”
- 关键生命周期事件可在日志中完整追踪
- 默认测试集无回归，新增 P1 相关测试通过
